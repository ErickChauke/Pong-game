...
int main()
{
	...
	if(isPlaying) {
	    ...
	    // Left Paddle
	    if(ball.getPosition().x - ballRadius < leftPaddle.getPosition().x + leftPaddleSize.x / 2 &&
	        ball.getPosition().x - ballRadius > leftPaddle.getPosition().x &&
	        ball.getPosition().y + ballRadius >= leftPaddle.getPosition().y - leftPaddleSize.y / 2 &&
	        ball.getPosition().y - ballRadius <= leftPaddle.getPosition().y + leftPaddleSize.y / 2) {
		...
		ballSound.play();
		ball.setPosition(
		    leftPaddle.getPosition().x + ballRadius + leftPaddleSize.x / 2 + 0.1f, ball.getPosition().y);
	    }
		...
		ballSound.play();
		ball.setPosition(
		    rightPaddle.getPosition().x - ballRadius - paddleSize.x / 2 - 0.1f, ball.getPosition().y);
	    }
	}
